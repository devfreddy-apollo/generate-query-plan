 ...
 */
export function assertUnreachableOrReturnDefault(unreachableValue, defaultValue) {
    if (process.env.NODE_ENV !== "production") {
        assertUnreachable(unreachableValue);
    }
    else {
        console.error(new Error(`Unreachable value ${JSON.stringify(unreachableValue)}, didn't expect to get here`));
        return defaultValue;
    }
}
