...
function handleUnknownNode(currentNode) {
    const nodeHash = hash();
    const unknownNode = {
        startHash: nodeHash,
        endHash: [nodeHash],
    ...
    switch (currentNode.kind) {
        case "Sequence": {
            ...
            return {
                ...
                nodeText: processedChildren
                    ...
                    if (i === 0)
                        return processedChild.nodeText;
        ...
        case "Fetch": {
            ...
            return {
                startHash: nodeHash,
                endHash: [nodeHash],
                nodeText: `${nodeHash}("Fetch (${currentNode.serviceName.split("?")[0] // adding support for connectors to strip args out of the path, which represents a subgraph
                })")`,
            };
        }
        case "Flatten": {
        ...
        case "Defer": {
            ...
            const primaryProcessed = currentNode.primary.node && processNode(currentNode.primary.node);
            return {
                startHash: nodeHash,
                endHash: [nodeHash],
                nodeText: `
        ...
        case "Subscription": {
            const nodeHash = hash();
            const processedChildren = (currentNode.rest?.nodes ?? []).map(processNode);
            return {
                startHash: nodeHash,
                endHash: processedChildren.flatMap((node) => node.endHash ?? []),
                nodeText: `
        ...
        case "Condition": {
            ...
            const primaryProcessed = processNode(currentNode.elseClause ?? currentNode.ifClause);
            return {
                startHash: nodeHash,
                endHash: [nodeHash],
                nodeText: `
