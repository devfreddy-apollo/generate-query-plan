...
export function getSchemaDirectives(
  ...
  return documentNode.definitions.flatMap((def) =>
    def.kind === "SchemaDefinition" || def.kind === "SchemaExtension"
      ? def.directives?.filter(
 ...
 * connect.
 */
export function hasConnectLink(documentNode: DocumentNode) {
  return getSchemaDirectives(documentNode, "link").some((linkDirective) => {
    const urlArg = linkDirective.arguments?.find(
      (arg) => arg.name.value === "url"
    )?.value;

    return (
      urlArg?.kind === "StringValue" &&
